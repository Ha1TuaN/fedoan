...
const ModalItem = (props) => {
      ...
      try {
        ...
        if (_data) {
          _data.categoryGroup = _data?.categoryGroupId
            ? {
                value: _data?.categoryGroupId ?? null,
                label: _data?.categoryGroupName ?? null,
              }
            : null;
  ...
  const onFinish = async () => {
    ...
    try {
      ...
      const res = id ? await requestPUT_NEW(`api/v1/categories/${id}`, formData) : await requestPOST_NEW(`api/v1/categories`, formData);
          ...
          {!loadding && (
            <Form form={form} initialValues={{isActive: true, sortOrder: count + 1}} layout='vertical' autoComplete='off'>
              <div className='row'>
                ...
                <div className='col-xl-6 col-lg-6'>
                  <FormItem label='Nhóm danh mục' name='categoryGroup' rules={[{required: true, message: 'Không được để trống!'}]}>
                    <TDSelect
                      placeholder=''
                      fetchOptions={async (keyword) => {
                        const res = await requestPOST(`api/v1/categorygroups/search`, {
                          pageNumber: 1,
                          pageSize: 1000,
                          advancedSearch: {
                            fields: ['name'],
                      ...
                      onChange={(value, current) => {
                        form.setFieldsValue({
                          categoryGroupId: value ? current?.id : null,
                          categoryGroupName: value ? current?.name : null,
                        });
                      }}
                    />
                  </FormItem>
