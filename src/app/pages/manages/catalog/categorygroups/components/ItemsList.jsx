...
const UsersList = (props) => {
  ...
  const handleButton = async (type, item) => {
    switch (type) {
      ...
      case 'delete':
        var res = await requestDELETE(`api/v1/categorygroups/${item.id}`);
        if (res) {
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (text, record) => {
        return <Checkbox checked={record.isActive ? true : false} disabled />;
      },
    },
    {
      title: 'Ghi chú',
  ...
  return (
    <>
      ...
      {modalVisible ? <ModalItem count={count} /> : <></>}
    </>
  );
};
