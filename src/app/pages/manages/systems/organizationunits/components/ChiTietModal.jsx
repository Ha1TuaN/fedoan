...
import {Form, Input, Spin, InputNumber, TreeSelect} from 'antd';
...
const ModalItem = (props) => {
  ...
  const id =
    modalOrganizationUnit?.type == 'suanhom' ? dataModal?.id ?? null : modalOrganizationUnit?.type == 'chitiet' ? dataModalChiTiet?.id ?? null : null;
          ...
          {!loading && (
            <Form form={form} layout='vertical' /* initialValues={initData} */ autoComplete='off'>
              <div className='row'>
                ...
                <div className='col-xl-6 col-lg-6'>
                  <FormItem label='Mã đơn vị' name='code' rules={[{required: true, message: 'Không được để trống!'}]}>
                    <Input placeholder='' />
                  </FormItem>
                </div>
                <div className='col-xl-6 col-lg-6'>
                ...
                <div className='col-xl-6 col-lg-6'>
                  <FormItem
                    label='Thuộc đơn vị'
                    name='parentId'
                    // rules={[{required: !(!currentParentId && id), message: 'Không được để trống!'}]}
                  >
                    <TreeSelect
                  ...
                  >
                    <TDSelect
                      ...
                      fetchOptions={async (keyword) => {
                        const res = await requestPOST(`api/v1/organizationunittypes/search`, {
                          pageNumber: 1,
                          pageSize: 1000,
                          orderBy: ['name '],
                        });
                      ...
                      onChange={(value, current) => {
                        if (value) {
                          form.setFieldValue('organizationUnitTypeId', current?.id);
                        } else {
